...
echo.
echo ================================================
echo   Basic RAD Server Interceptor Demo Tests (C++)
echo ================================================
echo.
...
echo [2/5] Testing GET /demo/hello with metadata=false
...
echo [3/5] Testing POST /demo/echo with valid JSON
...
echo [4/5] Testing POST /demo/echo with metadata=0
echo Expected: 201 - Echo response WITHOUT metadata
echo.
curl -X POST "http://localhost:8080/demo/echo?metadata=0" -H "Content-Type: application/json" -d "{\"message\":\"Hello World\"}"
echo.
echo.
